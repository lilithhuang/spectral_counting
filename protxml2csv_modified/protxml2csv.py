...
def convert_to_csv(params):
    ...
    for group_id, protxml_group in protxml_groups.items():
        ...
        for protein in protxml_group['proteins']:
            ...
            for peptide_sequence, peptide in protein['peptides'].items():
                peptide['scans'] = []
        ...
        for scan in pepxml_reader.iter():
            ...
            for match in scan['matches']:
                ...
                for seqid in seqids:
                    ...
                    if test_sequence in protein['peptides']:
                        peptide = protein['peptides'][test_sequence]
    ...
    for group_id, protxml_group in protxml_groups.items():
        ...
        for i in reversed(range(len(proteins))):
            ...
            if protein['probability'] < prob_cutoff or protein['protein_name'].startswith('DECOY'):
                ...
                for peptide_sequence, peptide in list(protein['peptides'].items()):
                    ...
                    if unique == True:
                        if peptide['is_nondegenerate_evidence'] == 'N':
                            del protein['peptides'][peptide['peptide_sequence']]
                            continue
...
#        print('Protein Probability Cutoff: ' + str(prob_cutoff))
    ...
    for group_id, protxml_group in protxml_groups.items():
        ...
        if protxml_groups[group_id]['cluster']['protein_group_id'] != protein['protein_name']:
            ...
            protxml_groups[group_id]['cluster']['description'] = 'Cluster of ' + protein['description']
            ...
            for peptide_sequence, peptide in protein['peptides'].items(): 
                ...
                if len(peptide['scans']) == 0: #handles case where peptide has no scans
                    ...
                    if peptide['peptide_sequence'] not in peptide_dict:
                        peptide_dict[peptide['peptide_sequence']] = {'protein_list': [], 'counts': {}, 'mods': []}
                    ...
                    for scan in peptide['scans']:
                        ...
                        if rep not in peptide_entry['counts']:
                            ...
                            if (scan['matches'][0]['modified_peptide'], scan['start_scan']) not in peptide_entry['counts'][rep]['modified_peptides']: 
                                peptide_entry['counts'][rep]['modified_peptides'].append((scan['matches'][0]['modified_peptide'], scan['start_scan'])) #list of modifications for peptide report
                            ...
                            if (scan['matches'][0]['modified_peptide'], scan['start_scan']) not in peptide_entry['counts'][rep]['modified_peptides']: 
                                peptide_entry['counts'][rep]['modified_peptides'].append((scan['matches'][0]['modified_peptide'], scan['start_scan']))
    ...
    for group_id, group in protxml_groups.items():
        ...
        protein = sorted(group['proteins'], key=lambda x:x['group_sibling_id'])[0]
        if group['cluster']['protein_group_id'] != protein['protein_name']:
            print (protein['protein_name'])
            protxml_groups[group_id]['cluster']['protein_group_id'] = protein['protein_name']
            protxml_groups[group_id]['cluster']['description'] = 'Cluster of ' + protein['description']
    ...
    for peptide_name, peptide in peptide_dict.items():          
        ...
        for rep in reps:
            counter_list.append(Counter([x[0] for x in peptide['counts'][rep]['modified_peptides']]) if rep in peptide['counts'] else 0)
